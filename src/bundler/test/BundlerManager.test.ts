...
import { TestFakeWalletToken__factory } from '../../contract-types';
import { DeterministicDeployer } from '../../sdk';
import type { UserOperation } from '../../utils';
import { AddressZero, getUserOpHash } from '../../utils';
import {
  ValidationManager,
...
describe('#BundlerManager', () => {
  ...
  describe('createBundle', function () {
    ...
    it('should not include a UserOp that accesses the storage of a different known sender', async function () {
      ...
      const wallet1 = await new TestFakeWalletToken__factory(signer).deploy(
        entryPoint.address,
      );
      const wallet2 = await new TestFakeWalletToken__factory(signer).deploy(
        entryPoint.address,
      );
      ...
      assert.equal(
        bundle!.userOpHashes[0],
        getUserOpHash(userOp1, entryPoint.address, await signer.getChainId()),
      );
