...
import type { TracerTest } from '../../utils';
import { TracerTest__factory } from '../../utils';
import type { BundlerTracerResult } from '../../validation-manager/BundlerCollectorTracer';
import { bundlerCollectorTracer } from '../../validation-manager/BundlerCollectorTracer';
import { debug_traceCall } from '../../validation-manager/GethTracer';
...
describe('#bundlerCollectorTracer', () => {
  ...
  before(async function () {
    ...
    if (ver.match('go1') == null) {
      console.warn(
        `\t==WARNING: test requires debug_traceCall on Geth (go-ethereum) node. ver=${ver}`,
      );
      this.skip();
      return;
    }
  ...
  it('should count opcodes on depth>1', async () => {
    const ret = await traceExecSelf(
      tester.interface.encodeFunctionData('callTimeStamp'),
      false,
      true,
    );
    const execEvent = tester.interface.decodeEventLog(
      'ExecSelfResult',
      ret.logs[0].data,
      ret.logs[0].topics,
    );
  ...
  ): Promise<BundlerTracerResult> {
    const ret: BundlerTracerResult = await debug_traceCall(
      provider,
      {
        to: tester.address,
        data: functionData,
   ...
   * @param extraWrapper
   */
  async function traceExecSelf(
    functionData: BytesLike,
    useNumber = true,
    extraWrapper = false,
  ): Promise<BundlerTracerResult> {
    const execTestCallGas = tester.interface.encodeFunctionData('execSelf', [
      functionData,
      useNumber,
    ]);
    if (extraWrapper) {
  ...
  describe('#traceExecSelf', () => {
    it('should revert', async () => {
      ...
      const log = tester.interface.decodeEventLog(
        'ExecSelfResult',
        ret.logs[0].data,
        ret.logs[0].topics,
      );
    ...
    it('should call itself', async () => {
      ...
      const execInner = tester.interface.encodeFunctionData('execSelf', [
        innerCall,
        false,
      ]);
      ...
      ret.logs.forEach((log) => {
        const logParams = tester.interface.decodeEventLog(
          'ExecSelfResult',
          log.data,
          log.topics,
        );
  ...
  it('should report direct use of GAS opcode', async () => {
    const ret = await traceExecSelf(
      tester.interface.encodeFunctionData('testCallGas'),
      false,
    );
  ...
  it('should ignore gas used as part of "call"', async () => {
    ...
    const callDoNothing = tester.interface.encodeFunctionData('execSelf', [
      doNothing,
      false,
    ]);
  ...
  it('should collect traces only until BeginExecution event', async () => {
    ...
    const callStopTracing =
      tester.interface.encodeFunctionData('testStopTracing');
