...
import { mergeStorageMap } from '../../utils';

describe('#moduleUtils', () => {
  describe('#mergeStorageMap', () => {
    it('merge item into empty map', () => {
      expect(mergeStorageMap({}, { a: 'val' })).to.eql({ a: 'val' });
    });
    it('merge items', () => {
      expect(
        mergeStorageMap({ a: 'vala', b: 'valb' }, { a: 'val', c: 'valc' }),
    ...
    it('should prefer root over slots in merged', async () => {
      expect(
        mergeStorageMap({ a: 'aa1' }, { a: { s1: 's1', s3: 'v3' } }),
      ).to.eql({ a: 'aa1' });
    });
    it('should prefer root over slots in validateStorage', async () => {
      expect(
        mergeStorageMap({ a: { s1: 's1', s3: 'v3' } }, { a: 'aa1' }),
      ).to.eql({ a: 'aa1' });
    });
  });
});
