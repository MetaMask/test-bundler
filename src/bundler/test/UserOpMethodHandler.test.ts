...
import type { SampleRecipient, TestRulesAccount } from '../../contract-types';
import { TestRulesAccount__factory } from '../../contract-types';
import { DeterministicDeployer, SimpleAccountAPI } from '../../sdk';
import { resolveHexlify, waitFor } from '../../utils';
import { postExecutionDump } from '../../utils/postExecCheck';
import {
  ValidationManager,
...
import type { UserOperationReceipt } from '../RpcTypes';
...
describe('UserOpMethodHandler', function () {
  ...
  before(async function () {
    ...
    accountDeployerAddress = await DeterministicDeployer.deploy(
      new SimpleAccountFactory__factory(),
      0,
      [entryPoint.address],
    );

    const sampleRecipientFactory = await ethers.getContractFactory(
      'SampleRecipient',
    );
  ...
  describe('eth_supportedEntryPoints', function () {
    it('eth_supportedEntryPoints', async () => {
      await expect(await methodHandler.getSupportedEntryPoints()).to.eql([
        entryPoint.address,
      ]);
    });
  });
    ...
    it('estimateUserOperationGas should estimate even without eth', async () => {
      ...
      expect(
        await methodHandler
          ...
          .catch((e) => e.message),
      ).to.eql("AA21 didn't pay prefund");
  ...
  describe('sendUserOperation', function () {
    ...
    before(async function () {
      ...
      userOperation = await resolveProperties(
        await smartAccountAPI.createSignedUserOp({
          data: sampleRecipient.interface.encodeFunctionData('something', [
            helloWorld,
          ]),
          target: sampleRecipient.address,
        }),
      );
      userOpHash = await methodHandler.sendUserOperation(
        await resolveHexlify(userOperation),
        entryPoint.address,
      );
    });
    ...
    it('should send UserOperation transaction to entryPoint', async function () {
      ...
      const logs = transactionReceipt.logs
        .filter((log) => log.address === entryPoint.address)
      ...
      const [senderEvent] = await sampleRecipient.queryFilter(
        sampleRecipient.filters.Sender(),
      ...
      assert.equal(
        senderEvent.args.txOrigin,
        expectedTxOrigin,
        'sample origin should be bundler',
      );
      assert.equal(
        senderEvent.args.msgSender,
        accountAddress,
        'sample msgsender should be account address',
      );
    });
    ...
    it('should expose FailedOp errors as text messages', async () => {
      ...
      try {
        await methodHandler.sendUserOperation(
          await resolveHexlify(op),
          entryPoint.address,
        );
    ...
    describe('validate get paid enough', function () {
      it('should pay just enough', async () => {
        ...
        const id = await methodHandler.sendUserOperation(
          await resolveHexlify(op),
          entryPoint.address,
        );
  ...
  describe('#_filterLogs', function () {
    ...
    it('should fail if no UserOperationEvent', async () => {
      expect(() => methodHandler._filterLogs(u1, [ev1])).to.throw(
        'no UserOperationEvent in logs',
      );
    });
    ...
    it('should return events for middle userOp in a bundle', async () => {
      expect(methodHandler._filterLogs(u1, [ev2, u2, ev1, u1, ev3, u3])).to.eql(
        [ev1],
      );
    });
  });
  ...
  describe('#getUserOperationReceipt', function () {
    ...
    it('should return null for nonexistent hash', async () => {
      expect(
        await methodHandler.getUserOperationReceipt(ethers.constants.HashZero),
      ).to.equal(null);
    });

    it('receipt should contain only userOp execution events..', async () => {
      expect(receipt.logs.length).to.equal(1);
      acc.interface.decodeEventLog(
        'TestMessage',
        receipt.logs[0].data,
    ...
    it('receipt should carry transaction receipt', () => {
      ...
      const logs = receipt.receipt.logs.filter(
        (log) => log.address !== '0x0000000000000000000000000000000000001010',
      );
